...
const Appointments = () => {
    ...
    const updatePaymentMutation = useMutation({
        mutationFn: ({ id, status }) => doctorApi.updatePaymentStatus(id, status),
        onSuccess: () => {
            queryClient.invalidateQueries(['doctorAppointments']);
        }
    });
                                            ...
                                            <div className="flex flex-col items-end gap-1 px-4 border-r border-slate-100">
                                                ...
                                                <button
                                                    onClick={() => updatePaymentMutation.mutate({
                                                        id: appt._id,
                                                        status: appt.paymentStatus === 'Paid' ? 'Unpaid' : 'Paid'
                                                    })}
                                                    disabled={updatePaymentMutation.isLoading}
                                                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${appt.paymentStatus === 'Paid'
                                                        ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200 hover:bg-emerald-600'
                                                        : 'bg-slate-100 text-slate-500 hover:bg-slate-200'
                                                        }`}
                                                >
                                                    {appt.paymentStatus === 'Paid' ? 'Marked Paid' : 'Mark as Paid'}
                                                </button>
                                            </div>
